...
export default function EnterpriseSecurity() {
  ...
  return <main className="container mx-auto max-w-7xl space-y-6 px-4 py-8" dir={locale === "ar" || locale === "ur" ? "rtl" : "ltr"}>
    ...
    {summary && <>
      ...
      <section data-testid="ai-phishing-admin-summary" aria-label="AI phishing summary" className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">تحليل التصيد بالذكاء الاصطناعي</h2><p className="mt-1 text-sm text-muted-foreground">يعرض هذا الملخص أعدادًا ودرجات فقط، ولا يكشف نص الرسائل لأعضاء المؤسسة.</p></div>{canManage && <Button size="sm" variant="outline" disabled={aiPhishingBusy} onClick={() => void toggleAiPhishing()}>{aiPhishingBusy ? "جارٍ الحفظ…" : summary.aiPhishing.enabled ? "إيقاف التحليل" : "تفعيل مع الموافقة"}</Button>}</div><div className="mt-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-6"><Metric label="الحالة" value={summary.aiPhishing.provider.state} /><Metric label="المحللة" value={summary.aiPhishing.analyzedMessages} /><Metric label="آمنة" value={summary.aiPhishing.safe} /><Metric label="مشبوهة" value={summary.aiPhishing.suspicious} /><Metric label="خطرة" value={summary.aiPhishing.dangerous} /><Metric label="متوسط الخطر" value={`${summary.aiPhishing.averageRiskScore}/100`} /></div></section>
