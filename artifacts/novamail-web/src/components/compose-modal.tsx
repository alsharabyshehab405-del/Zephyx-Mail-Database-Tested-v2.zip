...
export function ComposeModal({
  ...
  const [extendedAiOperation, setExtendedAiOperation] =
    React.useState<AiWriteOperation>("professional");
  const [templates, setTemplates] = React.useState<
    Array<{ id: string; name: string; subject: string; bodyHtml: string; bodyText: string }>
  >([]);
  const [undoToast, setUndoToast] = React.useState<{
    emailId: string;
    durationSeconds: number;
  } | null>(null);
  const [recipientChips, setRecipientChips] = React.useState<Record<"to" | "cc" | "bcc", string[]>>(
    { to: [], cc: [], bcc: [] },
  );
  const [recipientInputs, setRecipientInputs] = React.useState<Record<"to" | "cc" | "bcc", string>>(
    { to: "", cc: "", bcc: "" },
  );
  ...
  const syncRecipientField = (
    fieldName: "to" | "cc" | "bcc",
    chips: string[],
    input = recipientInputs[fieldName],
  ) => {
  ...
  const renderRecipientChipField = (
    fieldName: "to" | "cc" | "bcc",
    label: string,
    placeholder: string,
  ) => (
          ...
          <FormControl>
            <div className="flex min-h-8 flex-1 flex-wrap items-center gap-1.5">
              {recipientChips[fieldName].map((recipient) => {
                ...
                return (
                  <span
                    key={`${fieldName}-${recipient}`}
                    className={`inline-flex max-w-full items-center gap-1 rounded-full border px-2 py-1 text-xs ${valid ? "border-primary/20 bg-primary/10 text-foreground" : "border-destructive/50 bg-destructive/10 text-destructive"}`}
                  >
                    <bdi dir="ltr" className="max-w-[14rem] truncate">
                      {recipient}
                    </bdi>
                    <button
                      type="button"
                      ...
                      aria-label={`${t("email.removeRecipient")}: ${recipient}`}
                      onClick={() =>
                        syncRecipientField(
                          fieldName,
                          recipientChips[fieldName].filter((item) => item !== recipient),
                        )
                      }
                      disabled={isBusy}
                    >
              ...
              <Input
                ...
                aria-label={label}
                aria-invalid={
                  recipientInputs[fieldName].length > 0 &&
                  !areRecipientsValid(parseRecipients(recipientInputs[fieldName]))
                }
                placeholder={
                  recipientChips[fieldName].length ? t("email.addRecipient") : placeholder
                }
                ...
                onChange={(event) => {
                  ...
                  field.onChange(
                    [...recipientChips[fieldName], event.target.value].filter(Boolean).join(", "),
                  );
                }}
  ...
  const handleSaveOrSend = async (values: ComposeValues, sendNow: boolean) => {
    ...
    if (
      sendNow &&
      plannedDate &&
      (Number.isNaN(plannedDate.getTime()) || plannedDate.getTime() <= Date.now())
    ) {
    ...
    if (
      ![...recipients, ...ccRecipients, ...bccRecipients].every(
  ...
  const handleAiAssist = async (operation: AiWriteOperation) => {
    const currentBody =
      editorRef.current?.innerText || stripHtml(String(form.getValues("bodyText") || ""));
    ...
    try {
      ...
      if (result.state === "NOT_CONFIGURED" || !result.text) {
        toast({
          title: t("email.aiNotConfigured"),
          description: t("email.aiNotConfigured"),
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: t("email.reviewAiSuggestion"),
        description: t("email.aiActionRequiresConfirmation"),
      });
    } catch (error) {
      const notConfigured =
              ...
              <div className="novamail-compose-body flex-1 overflow-hidden flex flex-col">
                <div className="flex items-center gap-1 border-b px-3 py-2 bg-muted/10">
                  ...
                  <Button
                    ...
                    title={t("email.formatBulletedList")}
                  >
                  ...
                  <div className="ms-2 flex min-w-0 flex-1 items-center gap-1">
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="secondary"
                      onClick={() => void handleAiAssist("draft")}
                      disabled={isBusy}
                      className="h-8 px-2 text-xs"
                    >
                      {t("email.aiWriteAction")}
                    </Button>
                    <Button
                      type="button"
                      size="sm"
                      variant="ghost"
                      onClick={() => void handleAiAssist("rephrase")}
                      disabled={isBusy}
                      className="h-8 px-2 text-xs"
                    >
                      {t("email.rephraseAction")}
                    </Button>
                    <Button
                      type="button"
                      size="sm"
                    ...
                    {replyToId && (
                      <Button
                        ...
                        size="sm"
                        variant="ghost"
                        onClick={() => void handleAiAssist("quick_reply")}
                        disabled={isBusy}
                        className="h-8 px-2 text-xs"
                      >
                        {t("email.quickReplyAction")}
                      </Button>
                    )}
                    <select
                      value={extendedAiOperation}
                      onChange={(event) =>
                        setExtendedAiOperation(event.target.value as AiWriteOperation)
                      }
                      aria-label={t("email.aiExtendedOperation")}
                      disabled={isBusy}
                      className="h-8 max-w-36 rounded-md border bg-background px-1 text-xs"
                    >
                      <option value="expand">{t("email.aiExpand")}</option>
                      <option value="professional">{t("email.aiProfessional")}</option>
                      <option value="friendly">{t("email.aiFriendly")}</option>
                      <option value="formal">{t("email.aiFormal")}</option>
                      <option value="casual">{t("email.aiCasual")}</option>
                      <option value="polite">{t("email.aiPolite")}</option>
                      <option value="direct">{t("email.aiDirect")}</option>
                      <option value="grammar">{t("email.aiGrammar")}</option>
                      <option value="translate">{t("email.aiTranslate")}</option>
                      <option value="subject">{t("email.aiSubject")}</option>
                    </select>
                    <Button
                      type="button"
                      size="sm"
                      variant="ghost"
                      onClick={() => void handleAiAssist(extendedAiOperation)}
                      disabled={isBusy}
                      className="h-8 px-2 text-xs"
                    >
                      {t("email.aiRunExtended")}
                    </Button>
                    {templates.length > 0 && (
                      <select
                        aria-label={t("email.insertTemplate")}
                        defaultValue=""
                        onChange={(event) => handleTemplateInsert(event.target.value)}
                        disabled={isBusy}
                ...
                {aiSuggestion ? (
                  <div
                    className="border-b border-primary/20 bg-primary/[0.04] p-3"
                    role="region"
                    aria-label={t("email.reviewAiSuggestion")}
                  >
                    <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                      <p className="text-xs font-semibold text-primary">
                        {t("email.reviewAiSuggestion")}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("email.aiActionRequiresConfirmation")}
                      </p>
                    </div>
                    <p
                      className="max-h-32 overflow-y-auto whitespace-pre-wrap rounded-lg border border-border/60 bg-background p-3 text-sm"
                      dir="auto"
                    >
                      {aiSuggestion.text}
                    </p>
                    <div className="mt-2 flex flex-wrap justify-end gap-2">
                      <Button
                        ...
                        disabled={isBusy}
                      >
                        {t("email.cancelAiSuggestion")}
                      </Button>
                      <Button type="button" size="sm" onClick={applyAiSuggestion} disabled={isBusy}>
                        {t("email.applyAiSuggestion")}
                      </Button>
                    </div>
                  </div>
                ) : null}
              ...
              <div className="compose-footer novamail-compose-footer p-3 border-t bg-muted/20 flex flex-wrap justify-between items-center gap-2">
                ...
                <div className="compose-footer__primary flex items-center gap-2 flex-wrap justify-end">
                  <details className="relative rounded-lg border border-border/60 bg-background px-2 py-1">
                    <summary className="cursor-pointer list-none text-xs font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                      {t("email.deliveryOptions")}
                    </summary>
                    <div className="mt-2 flex flex-col gap-2 border-t border-border/60 pt-2 text-xs text-muted-foreground sm:absolute sm:bottom-full sm:end-0 sm:mb-2 sm:w-72 sm:rounded-xl sm:border sm:bg-card sm:p-3 sm:shadow-lg">
                      ...
                      {!scheduledAt && (
                        <label
                          ...
                          htmlFor="undo-send-delay"
                        >
                          <span>{t("email.undoSendDelay")}</span>
                          <select
                            id="undo-send-delay"
                            aria-label={t("email.undoSendDelay")}
