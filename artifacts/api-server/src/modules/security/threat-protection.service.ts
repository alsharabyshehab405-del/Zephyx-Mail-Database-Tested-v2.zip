...
export function securityProviderStatus() {
  const clamavConfigured = process.env.ATTACHMENT_SCANNING_ENABLED?.trim().toLowerCase() === "true" && Boolean((process.env.CLAMAV_HOST && process.env.CLAMAV_PORT) || process.env.CLAMAV_SOCKET);
