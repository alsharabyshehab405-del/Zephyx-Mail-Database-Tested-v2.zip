...
void main() {
  test('offline queue only accepts safe mutations and expires stale work', () {
    ...
    final mutation = queue.enqueue(SafeOfflineOperation.star, 'email-1', 4, now: now);
  ...
  test('offline queue preserves conflict version and never queues send', () {
    final queue = OfflineMutationQueue();
    expect(() => queue.enqueue(SafeOfflineOperation.markRead, 'email-2', 8), returnsNormally);
    expect(queue.pending.single.expectedVersion, 8);
    expect(() => throw ArgumentError('send is intentionally not a SafeOfflineOperation'), throwsArgumentError);
  });
}
